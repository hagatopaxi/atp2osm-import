...
Les worktrees git vivent **toujours** dans `.worktrees/<nom>` à la racine du projet, jamais ailleurs (surtout pas dans `.claude/`). `dev.sh` résout les noms de worktree depuis ce répertoire.

## Commands

```bash
# Lancer l'app pour un worktree (port dérivé du nom, .env symlinké depuis le checkout principal)
./dev.sh up [-d] [nom]     # démarrer (-d = détaché)
./dev.sh down [nom]        # arrêter
./dev.sh logs [nom] [-f]   # voir les logs
